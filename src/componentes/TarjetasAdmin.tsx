...
export default function TarjetasAdmin({ filtros }: TarjetasAdminProps) {
  ...
  const obtenerReportes = async () => {
    const reportesRef = collection(db, "reportes_publicaciones");
  ...
  const eliminarReporte = async (id: string) => {
    const ref = doc(db, "reportes_publicaciones", id);
    await deleteDoc(ref);
    obtenerReportes();
  };
              ...
              <tr key={u.id} className="border-t">
                <td className="p-2">
                  <Input
                    value={u.nombre}
                    onChange={(e) =>
                      editarUsuario(u.id, "nombre", e.target.value)
                    }
                  />
                </td>
                <td className="p-2">
                  <Input
                    value={u.apellido}
                    onChange={(e) =>
                      editarUsuario(u.id, "apellido", e.target.value)
                    }
                  />
                </td>
                <td className="p-2">
                ...
                <td className="p-2">
                  <Input
                    value={u.carrera}
                    onChange={(e) =>
                      editarUsuario(u.id, "carrera", e.target.value)
                    }
                  />
                </td>
                <td className="p-2">
          ...
          <tbody>
            {reportes.map((r) => (
              <tr key={r.id} className="border-t">
                ...
                <td className="p-2">{r.fecha.toDate().toLocaleString()}</td>
